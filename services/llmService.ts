...
export async function callTranslationQaLLM(payload: LlmRequestPayload): Promise<LlmResponse> {
  ...
  const runAnalysis = async (modelId: string): Promise<ScreenshotReport> => {
    ...
    const userPrompt = `
      ...
      4. If it IS a variable, you MUST preserve its integrity. Suggest modifications that can be applied directly to the source template containing the placeholder (e.g., "Le Centre d'assistance {{$Brand_DisplayName}}").
