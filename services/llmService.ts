...
4. Variable Integrity:
   - If a source structure image or reference is provided, ensure any suggested fixes for variable-driven text do not break or translate the variable placeholders themselves.

LQA SKILLS REFERENCE:
${skillsContext}

CRITICAL: ALL text fields in your JSON response (like 'reason' and 'refinedRationale') MUST be written in ${payload.reportLanguage === 'zh' ? 'Simplified Chinese (简体中文)' : 'English'}.
  `;
...
Please verify each issue and return the verdict.
  ...
  if (sourceImage) {
    parts.push({ inlineData: { mimeType: sourceImage.mimeType, data: sourceImage.data } });
  }
...
export async function callTranslationQaLLM(payload: LlmRequestPayload): Promise<LlmResponse> {
  ...
  const runAnalysis = async (modelId: string): Promise<ScreenshotReport> => {
    ...
    let sourceImage: ProcessedImage | null = null;
    if (payload.sourceStructureImage) {
      sourceImage = await processImageUrl(payload.sourceStructureImage);
    }
    ...
    const userPrompt = `
      ...
      ${(payload.sourceStructureRef || payload.sourceStructureImage) ? `
      Source Structure Snapshot / Variable Reference:
      ${payload.sourceStructureRef || "(See attached Image 3 for structure/variable reference)"}

      CRITICAL RULES FOR VARIABLES:
      1. Identify text generated by variables/placeholders from the reference above (e.g., "{{$Brand_DisplayName}}").
      2. NEVER change the internal word order, partially translate, or split these variables. Treat them as an indivisible black box.
      3. If target language grammar (like French) requires reordering nouns/modifiers (e.g., "A B Center" -> "Center A B"), verify if "A B" is a variable.
      4. If it IS a variable, you MUST preserve its integrity. Suggest modifications that can be applied directly to the source template containing the placeholder (e.g., "Le Centre d'assistance {{$Brand_DisplayName}}"). DO NOT replace the variable with its runtime text in your suggestion!
      5. Glossary Priority vs Variables: If glossary rules conflict with variable content, variable integrity is the HIGHEST priority.
      ` : ''}
      ...
      ${sourceImage ? '- Image 3: Source Structure Snapshot / Variable Reference' : ''}
      ...
      CRITICAL: ALL text fields in your JSON response (descriptions, summaries, rationales, etc.) MUST be written in ${payload.reportLanguage === 'zh' ? 'Simplified Chinese (简体中文)' : 'English'}, regardless of the target language being tested.
    `;
    ...
    if (parsedReport.issues.length > 0) {
      ...
      parsedReport = await verifyIssues(payload, parsedReport, enImage, deImage, ai, skillsBlock, sourceImage);
    }
